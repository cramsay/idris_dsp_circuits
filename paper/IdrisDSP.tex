...
\newcommand\cut[1]{\textcolor{purple}{#1}}
\usepackage{newfloat}
...
%% Write an abstract and then colourise parts I think I could remove.
\begin{abstract}

  We explore the use of dependent types to address the disparity between the
  theory and the practical description of many DSP circuits. After discussing an
  approach to modeling synchronous circuit behaviour in Idris (a pure
  functional language with dependent types) two DSP case studies are introduced
  --- an FIR filter with optimal word lengths and a CIC decimator with register
  pruning. Both of these scenarios prove difficult to describe in a
  parameterised fashion using traditional HDLs. While some implementations
  opt to use software programs to generate circuits, given a set of parameters,
  this introduces its own difficulties during verification. Instead, this work
  demonstrates that these circuits are readily described in an environment with
  dependent types.
...
When describing digital signal processing (DSP) circuits, there is often a
disparity between the mathematical theory and any practical descriptions in
Hardware Description Languages (HDLs). HDLs such as VHDL and Verilog do not
provide the language features required to fully parameterise some simple
structures, such as direct-form Finite Impulse Response (FIR) filters with
optimal word length growth.

\cut{
A consequence of this is that designers opt to either hand-craft an ad-hoc model
for their particular parameters or turn to software programming, in an attempt
to generate hardware descriptions in a fully parameterised way. Traditionally,
Perl has been a common choice for such a task, however this offers extremely
little in terms of static checking of the circuit generators --- placing a huge
extra burden of testing on the designer. This can also be problematic for
designers when evaluating $3^{\text{rd}}$ party IP; the evidence of testing is
vital in the complete absence any of invariants checked by a compiler. This
issue is even present in HDLs embedded in host languages with more expressive
type systems, such as Lava\cite{gill_09}, hosted in Haskell.
}

This paper introduces pure, functional programming languages with dependent
types as a solution to describing parameterised DSP circuits faithfully, and
with structures verified at compile-time. Dependent types allow the \emph{type}
of a program (in this case, encoding the structure of a circuit) to depend on
the \emph{values} of its arguments, instead of just the argument types (i.e. the
value of a coefficient, not just its word length). We use the language
Idris\cite{brady_11} to make this demonstration of modeling/simulating DSP
circuits --- currently without circuit synthesis.
...
\begin{enumerate}
\item How to best represent numeric types?
\item How to best model synchronous logic?
\end{enumerate}
...
As an introductory example, consider a direct form Finite Impulse Response
...
$m$-bit word is a $[\text{Max}(n,m)+1]$-bit word. This sort of growth can be captured
...
\item Often the coefficients will be constant. In this case the bit growth due
  to multiplication should vary with the numerical value of each constant
  coefficient.
\end{enumerate}
...
Now we consider how Idris can be applied to these challenges.
...
the full precision signals will be shortened to avoid excessive resource usage.
In this case, the designer may employ multiple strategies:
...
stages, followed by a $\frac{1}{R}$ rate downsampler, followed by $N$ comb
...
cannot perform an equivalent optimisation given an non-pruned description. This
...
input/output word lengths. Another function \texttt{prunej} inverts \texttt{bj}
to give the number of bits to retain, rather than discard. This function can now
be lifted up to the type definition of our CIC decimator circuit, letting the
compiler confirm that each CIC stage is pruned to meet the error variance design
constraint.
...
can even help guide the implementation of complex structures.
